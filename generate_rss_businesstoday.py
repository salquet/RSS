#!/usr/bin/env python3
"""RSS Generator for businesstoday.in"""
...
NEWS_URL = "https://www.businesstoday.in/"
OUTPUT_FILE = "businesstoday.xml"
...
def extract(soup):
    articles, seen = [], set()
    for a in soup.find_all("a", href=lambda h: h and "businesstoday.in" in h and "/story/" in h):
        url = a["href"].split("?")[0]
        if url in seen: continue
        title = a.get("title","") or a.get_text(strip=True)
        if not title or len(title) < 10: continue
        seen.add(url)
        articles.append({"title":title,"url":url,"date":date_from_url(url)})
    return articles

def build_rss(articles):
    rss = ET.Element("rss", version="2.0"); rss.set("xmlns:atom","http://www.w3.org/2005/Atom")
    ch = ET.SubElement(rss,"channel")
    ET.SubElement(ch,"title").text="Business Today India"
    ET.SubElement(ch,"link").text=NEWS_URL
    ET.SubElement(ch,"description").text="Business and financial news from India"
